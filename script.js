...
function processSupportChat(input) {
    ...
    if (inputLower.includes('契約') || inputLower.includes('申込') || inputLower.includes('新規') || inputLower.includes('解約')) {
        ...
        addBotMessage(generalResponses.outage || '停電情報についてお答えいたします。<br><br>現在、お客様の地域での停電は報告されておりません。<br><br>もし電気がご使用になれない場合は、以下をご確認ください：<br>・ブレーカーが落ちていないか<br>・電気料金のお支払い状況<br><br>それでも解決しない場合は、緊急連絡先までお電話ください。<br><br>緊急時連絡先: 0120-986-777（24時間対応）');
        ...
        if (supportResponses[topic] && supportResponses[topic][currentStep + 1]) {
            ...
            // 会話終了
            addBotMessage(generalResponses.end || '他にご質問やご不明な点がございましたら、お気軽にお聞かせください。<br><br>詳しいご相談については、お電話でも承っております。<br><br>フリーダイヤル: 0120-986-302（平日9:00-18:00）');
        }
    } else {
        // 一般的な応答
        addBotMessage(generalResponses.default || '申し訳ございません。もう一度詳しく教えていただけますか？<br><br>以下のようなご質問にお答えできます：<br><br>・電力契約について<br>・料金プランについて<br>・ご利用料金について<br>・停電情報について<br><br>または、お電話でのご相談も承っております。<br>フリーダイヤル: 0120-986-302（平日9:00-18:00）');
    }
}
...
function updateMonthDisplay(monthIndex) {
    ...
    // 月のデータ（JSONファイルから読み込み）
    const monthsData = window.appData?.monthlyData?.monthsData || [
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentYear = window.appData?.monthlyData?.currentYear || 2025;

    function updateYearDisplay() {
        if (yearlyPeriodLabel) {
            const yearlyPeriod = window.appData?.monthlyData?.yearlyPeriod || `${currentYear}年8月～${currentYear + 1}年7月`;
            yearlyPeriodLabel.textContent = yearlyPeriod;
        }
...
function openNotificationDetail(notificationType) {
    switch(notificationType) {
        case 'campaign':
            ...
            break;
        case 'eco':
            const ecoDetail = window.appData?.notifications?.notificationDetails?.eco;
            alert(ecoDetail?.content || '省エネチャレンジの詳細ページに移動します。\n\n【省エネチャレンジとは】\n・月間の電気使用量を前年同月比で削減\n・削減率に応じてポイントを獲得\n・参加者全員に参加賞あり\n\n【今月の目標】\n・前年同月比5%削減で50ポイント\n・10%削減で100ポイント\n・15%削減で200ポイント\n\n現在の削減率：12%（目標達成済み！）');
            break;
        case 'game':
            const gameDetail = window.appData?.notifications?.notificationDetails?.game;
            alert(gameDetail?.content || '省エネゲームの詳細ページに移動します。\n\n【省エネゲームとは】\n・省エネに関する知識を楽しく学べるゲーム\n・クイズ形式で省エネのコツを学習\n・正解するとポイントを獲得\n\n【ゲーム内容】\n・省エネクイズ：5問のクイズに挑戦\n・制限時間：30秒/問\n・獲得ポイント：正解1問につき10ポイント\n\n【今すぐプレイ】\necoアプリのポイント確認ページからゲームを開始できます！');
            break;
        default:
            alert('詳細ページに移動します');
    }
